...
def grx_dnn_bin(train_data, val_data, test_data, t_nodes, t_fig, s_k, n_layer, n_node, n_lr, n_epoch, patiences,show_val=True):
    if show_val == True:
        print('DNN_iteration')
    ...
    for epoch in range(n_epoch):
        ...
        g_TX = model(torch.cat((R_0_train.unsqueeze(1), X_train), dim=1)).squeeze()
        ...
        with torch.no_grad():
            g_TX_val = model(torch.cat((R_0_val.unsqueeze(1), X_val), dim=1)).squeeze()
    ...
    with torch.no_grad():
        # IBS
        S_T_X_ibs = torch.ones(len(s_k), len(R_0_train), dtype=torch.float32) # 100 * 500
        for k in range(len(s_k)):
            s_k_k_repeat = torch.full((len(R_0_train),), s_k[k], dtype=torch.float32)
    ...
    with torch.no_grad():
        ...
        exp_g_TX_train = batch_func_train(nodes) #hazard n*100
        g_RX = model(torch.cat((R_0_train.unsqueeze(1), X_train), dim=1)).squeeze()
        g_R_X_train = g_RX
        int_exp_g_RX_train = vectorized_gaussian_quadrature_integral(batch_func_train, torch.zeros_like(R_0_train), R_0_train, n_points=100) # batch_size = 0.2n
        S_R_X = torch.exp(-int_exp_g_RX_train)
        Lambda_R_X = torch.exp(g_R_X_train)
    
    return {
        'S_R_X_train': S_T_X_ibs.T.detach().numpy(),  #  n * 100
        'Lambda_R_X_train': exp_g_TX_train.detach().numpy(), #  n * 100
        'S_R_X': S_R_X.detach().numpy(), 
        'Lambda_R_X': Lambda_R_X.detach().numpy(),
        'model': model
    }
...
def grax_dnn_bin(train_data, val_data, test_data, t_nodes, t_fig, s_k, n_layer, n_node, n_lr, n_epoch, patiences,show_val=True):
    ...
    with torch.no_grad():
        # Survival, train data
        S_T_X_ibs = torch.ones(len(s_k), len(R_O_train), dtype=torch.float32) # 100 * 500
        for k in range(len(s_k)):
            s_k_k_repeat = torch.full((len(R_O_train),), s_k[k], dtype=torch.float32)
            int_exp_g_TX = vectorized_gaussian_quadrature_integral(batch_func_train, torch.zeros_like(s_k_k_repeat), s_k_k_repeat, n_points=100,A=A_train)
            S_T_X_ibs[k] = torch.exp(-int_exp_g_TX)
    # with torch.no_grad():
    #     # Survival, treated
    ...
    with torch.no_grad():
        ...
        exp_g_TX_train = batch_func_train(nodes, A_train) #hazard 500*100
        # exp_g_TX_treated = batch_func_test(nodes, A_treated) #hazard
        g_RX = model(torch.cat((R_O_train.unsqueeze(1), X_train), dim=1)).squeeze()
        g_R_X_train = torch.sum(g_RX*A_train,dim=1)
        int_exp_g_RX_train = vectorized_gaussian_quadrature_integral(batch_func_train, torch.zeros_like(R_O_train), R_O_train, n_points=100,A=A_train) # batch_size = 0.2n
        S_R_X = torch.exp(-int_exp_g_RX_train)
        Lambda_R_X = torch.exp(g_R_X_train)
    # with torch.no_grad():
    #     #conditional mean
    ...
    return {
        ...
        # 'Cmean_R_X_untreat': Cmean_untreat.detach().numpy(), # n*1
...
#%% ----------Input:(t,X)-- Output:g(t,X) --------------
def grax1_dnn_bin(train_data, val_data, test_data, t_nodes, t_fig, s_k, n_layer, n_node, n_lr, n_epoch, patiences,show_val=True):
    if show_val == True:
        print('DNN_iteration')
    # Convert training and test data to tensors
    X_train = torch.tensor(train_data['X'], dtype=torch.float32)
    ...
    with torch.no_grad():
        ...
        exp_g_TX_train = batch_func_train(nodes) #hazard n*100
        g_RX = model(torch.cat((R_0_train.unsqueeze(1),A_train.unsqueeze(1), X_train), dim=1)).squeeze()
        g_R_X_train = g_RX
        int_exp_g_RX_train = vectorized_gaussian_quadrature_integral(batch_func_train, torch.zeros_like(R_0_train), R_0_train, n_points=100) # batch_size = 0.2n
        S_R_X = torch.exp(-int_exp_g_RX_train)
        Lambda_R_X = torch.exp(g_R_X_train)
