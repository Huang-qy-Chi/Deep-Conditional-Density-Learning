...
from data_generator import gendata_Linear_ed, gendata_Deep_ed
import warnings
...
def _one_run_linear(i, Seed, n, n_v, corr, beta, gamma, r_grid, test_data, t_nodes, t_fig,\
    ...
    train_data = gendata_Linear_ed(n=n, corr=corr, beta=beta, gamma=gamma)
    val_data   = gendata_Linear_ed(n=n_v, corr=corr, beta=beta, gamma=gamma)
...
def _one_run_deep(i, Seed, n, n_v, corr, r_grid, test_data, t_nodes, t_fig,\
    ...
    train_data = gendata_Deep_ed(n=n, corr=corr)
    val_data   = gendata_Deep_ed(n=n_v, corr=corr)
    St = DCDL(train_data, val_data, test_data, t_nodes, t_fig,\
               s_k, n_layer, n_node, n_lr, n_epoch, patiences,show_val=False)
...
if __name__ == '__main__':
    ...
    Seed = 1145
    set_seed(4514)
    test_data = gendata_Linear_ed(n = n1, corr = corr,beta=beta,gamma=gamma)
    ...
    print(f"{"Est_d"}_{15}_{"n"}_{n}_{"case"}_{"Linear"}")
    ...
    filename = f"{"Est_d"}_{15}_{"n"}_{n}_{"pr"}_{"Linear_2"}.npy"
    ...
    Seed = 1145
    set_seed(4514)
    test_data = gendata_Deep_ed(n = n1, corr = corr)
    start_time = time.time()
    ...
    print(f"{"Est_d"}_{15}_{"n"}_{n}_{"case"}_{"Deep"}")
    print(f"ISE_DCDL: {ISE_DCDL:.3f}, SE_ISE_DCDL: {SE_ISE_DCDL:.3f}", end='\n')
    print(f"ISE_KCDE: {ISE_KCDE:.3f}, SE_ISE_KCDE: {SE_ISE_KCDE:.3f}", end='\n')
    print(f"ISE_QRF: {ISE_QRF:.3f}, SE_ISE_QRF: {SE_ISE_QRF:.3f}", end='\n')
    print(f"ISE_FLEX: {ISE_FLEX:.3f}, SE_ISE_FLEX: {SE_ISE_FLEX:.3f}", end='\n')
    ...
    filename = f"{"Est_d"}_{15}_{"n"}_{n}_{"pr"}_{"Deep_2"}.npy"
