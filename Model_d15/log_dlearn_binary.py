...
def RDL_bin(train_data,val_data,test_data,alphas=None,cv=5,precompute = True):
    if alphas is None:
    ...
    mu_model = LassoCV(alphas=alphas, cv = cv, random_state=42, n_jobs=-1,precompute=precompute)
    mu_model.fit(X_train, logR_train)
    mu_pred_train = mu_model.predict(X_train)
    mu_pred_test = mu_model.predict(X_test)
    ...
    tau_model_cv = LassoCV(alphas=alphas, cv=cv, random_state=42, n_jobs=-1,precompute=False)
